...
const ProductCard = ({ productKey, name, type, price, images, sizes, playerName = "Messi", playerNumber = "10", badgeText = "Mundial 2026" }: ProductCardProps) => {
  ...
  const { getQty, loading: stockLoading } = useStock();

  const allOutOfStock = !stockLoading && sizes.every((s) => getQty(productKey, s) === 0);
  ...
  const handleAddToCart = () => {
    ...
    addItem({ productKey, name, type, size: selectedSize, price, image: images[0] });
    setJustAdded(true);
      ...
      <div className="p-6 space-y-5 flex-1 flex flex-col">
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            {sizes.map((size) => {
              const qty = getQty(productKey, size);
              const isOut = !stockLoading && qty === 0;
              ...
              return (
                <button
                  ...
                  disabled={isOut}
                  title={isOut ? "Sin stock" : qty <= 3 ? `Quedan ${qty}` : undefined}
                  className={`flex-1 py-2.5 rounded-lg text-sm font-semibold border transition-all duration-200 relative ${
                    isOut
                      ? "bg-secondary/40 text-muted-foreground/50 border-border/40 cursor-not-allowed line-through"
                      : isSelected
                ...
                >
                  {size}
                  {!isOut && !stockLoading && qty > 0 && qty <= 3 && (
        ...
        <AnimatePresence mode="wait">
          <motion.button
            key={justAdded ? "added" : allOutOfStock ? "out" : "add"}
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            exit={{ opacity: 0, scale: 0.95 }}
            onClick={handleAddToCart}
            disabled={!selectedSize || justAdded || allOutOfStock}
        ...
        <div className="flex items-center gap-2 justify-center">
          <div className={`h-2 w-2 rounded-full ${allOutOfStock ? "bg-destructive" : "bg-green-500 animate-pulse"}`} />
          <span className="text-xs text-muted-foreground">
            {allOutOfStock ? "Sin stock disponible" : "Stock limitado — últimas unidades"}
          </span>
        </div>
      </div>
    </motion.div>
