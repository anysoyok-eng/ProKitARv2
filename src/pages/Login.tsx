...
const Login = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!roles || roles.length === 0) {
      setError("No tenés permisos de administrador. Pedí que te asignen el rol admin en la base de datos.");
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    setSuccess(
      signUpData.session
        ? "¡Cuenta creada! Ahora necesitás que te asignen el rol admin en la base de datos."
        : "Te enviamos un email de confirmación. Confirmalo y después iniciá sesión para pedir acceso admin."
    );
    setIsRegister(false);
    setLoading(false);
  };
      ...
      >
        ...
        <div className="text-center space-y-3">
          ...
          <h1 className="font-heading text-2xl font-bold uppercase text-foreground">
            {isRegister ? "Registrarse" : "Admin Login"}
          </h1>
