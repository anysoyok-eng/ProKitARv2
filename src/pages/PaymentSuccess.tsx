...
const PaymentSuccess = () => {
  ...
  const [data, setData] = useState<VerifyResponse | null>(
    redirectStatus ? createFallbackResponse(redirectStatus) : null
  );
  const [isLoading, setIsLoading] = useState(!redirectStatus);
  const [isError, setIsError] = useState(false);
  const [retryCount, setRetryCount] = useState(0);
  const [usedRedirectFallback, setUsedRedirectFallback] = useState(Boolean(redirectStatus));
  ...
  // Corre en background — NO bloquea el render de la pantalla de éxito.
  const updateOrderStatus = useCallback(async (externalReference: string) => {
    ...
    // Espera la sesión con timeout corto (máx ~1s) para no demorar nada.
    ...
    if (!session) {
      console.warn("No auth session available to update order status (background)");
  ...
  useEffect(() => {
    // Si el redirect ya nos dio el status, confiamos en él y solo actualizamos la orden.
    ...
    // Sin status en la URL ni payment_id: no hay nada que verificar.
    if (!paymentId) {
      setIsLoading(false);
      return;
    }

    // Caso raro: tenemos paymentId pero no status. Marcamos como aprobado por fallback.
    setData(createFallbackResponse("approved"));
    setUsedRedirectFallback(true);
    setIsLoading(false);
  }, [paymentId, redirectStatus, searchParams, updateOrderStatus]);
